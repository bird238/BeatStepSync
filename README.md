...
- **Randomization Reverting to Old Values:** `Rnd Notes` / `Rnd Gates` used to fire up to 16 SysEx writes back-to-back with no gap between them, which could outrun the hardware's own receive/processing rate and silently drop some writes -- the poller's next pass would then read the un-written old value back, making the change look like it "reverted." Each write in these loops is now paced ~8ms apart.
